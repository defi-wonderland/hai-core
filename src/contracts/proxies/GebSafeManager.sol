...
contract GebSafeManager {
  ...
  function allowSAFE(uint256 safe, address usr, uint256 ok) public safeAllowed(safe) {
  ...
  function modifySAFECollateralization(uint256 safe, int256 deltaCollateral, int256 deltaDebt) public safeAllowed(safe) {
    address safeHandler = safes[safe];
    SAFEEngine(safeEngine).modifySAFECollateralization(
      collateralTypes[safe], safeHandler, safeHandler, safeHandler, deltaCollateral, deltaDebt
    );
  ...
  function transferCollateral(uint256 safe, address dst, uint256 wad) public safeAllowed(safe) {
    SAFEEngine(safeEngine).transferCollateral(collateralTypes[safe], safes[safe], dst, wad);
  ...
  function transferCollateral(bytes32 collateralType, uint256 safe, address dst, uint256 wad) public safeAllowed(safe) {
    SAFEEngine(safeEngine).transferCollateral(collateralType, safes[safe], dst, wad);
  ...
  function transferInternalCoins(uint256 safe, address dst, uint256 rad) public safeAllowed(safe) {
    SAFEEngine(safeEngine).transferInternalCoins(safes[safe], dst, rad);
  ...
  function quitSystem(uint256 safe, address dst) public safeAllowed(safe) handlerAllowed(dst) {
    (uint256 lockedCollateral, uint256 generatedDebt) = SAFEEngine(safeEngine).safes(collateralTypes[safe], safes[safe]);
    ...
    SAFEEngine(safeEngine).transferSAFECollateralAndDebt(
      collateralTypes[safe], safes[safe], dst, deltaCollateral, deltaDebt
    );
  ...
  function enterSystem(address src, uint256 safe) public handlerAllowed(src) safeAllowed(safe) {
    (uint256 lockedCollateral, uint256 generatedDebt) = SAFEEngine(safeEngine).safes(collateralTypes[safe], src);
    int256 deltaCollateral = toInt(lockedCollateral);
    int256 deltaDebt = toInt(generatedDebt);
    SAFEEngine(safeEngine).transferSAFECollateralAndDebt(
      collateralTypes[safe], src, safes[safe], deltaCollateral, deltaDebt
    );
  ...
  function moveSAFE(uint256 safeSrc, uint256 safeDst) public safeAllowed(safeSrc) safeAllowed(safeDst) {
    ...
    (uint256 lockedCollateral, uint256 generatedDebt) =
      SAFEEngine(safeEngine).safes(collateralTypes[safeSrc], safes[safeSrc]);
    int256 deltaCollateral = toInt(lockedCollateral);
    int256 deltaDebt = toInt(generatedDebt);
    SAFEEngine(safeEngine).transferSAFECollateralAndDebt(
      collateralTypes[safeSrc], safes[safeSrc], safes[safeDst], deltaCollateral, deltaDebt
    );
    emit MoveSAFE(msg.sender, safeSrc, safeDst);
  }

  // Choose a SAFE saviour inside LiquidationEngine for the SAFE with id 'safe'
  function protectSAFE(uint256 safe, address liquidationEngine, address saviour) public safeAllowed(safe) {
    LiquidationEngine(liquidationEngine).protectSAFE(collateralTypes[safe], safes[safe], saviour);
